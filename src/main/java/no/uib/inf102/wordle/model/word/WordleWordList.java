...
public class WordleWordList {
     ...
     * <code>feedback</code>. The method checks each candidate word against
     * the feedback from the last guess to determine which words are still valid
     * possible answers.
     * 
     * @param feedback The feedback from the last guess, which indicates
     *                 correct and misplaced letters.
     */
	public void eliminateWords(WordleWord feedback) {
		List<String> newPossibleAnswers = new ArrayList<>(); 
		
		for (String candidate : possibleAnswers) { // m
			
			if (WordleWord.isPossibleWord(candidate, feedback)) { // k
				newPossibleAnswers.add(candidate); 
			}
		}
